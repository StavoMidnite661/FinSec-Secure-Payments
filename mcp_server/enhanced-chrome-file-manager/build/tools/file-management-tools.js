...
export class FileManagementTools {
    ...
    async callTool(name, args) {
        switch (name) {
            case 'file_read':
                return this.readFile(args.filePath, args.encoding);
            case 'file_write':
                return this.writeFile(args.filePath, args.content, args.encoding, args.createDirs);
            case 'file_copy':
                return this.copyFile(args.sourcePath, args.destinationPath, args.overwrite);
            case 'file_move':
                return this.moveFile(args.sourcePath, args.destinationPath, args.overwrite);
            case 'file_delete':
                return this.deleteFile(args.filePath, args.recursive);
            case 'file_list':
                return this.listDirectory(args.dirPath, args.recursive, args.pattern);
            case 'file_info':
                return this.getFileInfo(args.filePath);
            case 'file_search':
                return this.searchFiles(args.pattern, args.rootDir, args.maxResults);
            case 'file_mkdir':
                return this.createDirectory(args.dirPath, args.recursive);
            case 'file_watch':
                return this.watchDirectory(args.dirPath, args.pattern);
            default:
    ...
    async writeFile(filePath, content, encoding = 'utf8', createDirs = true) {
        try {
            await this.validatePath(filePath);
            if (createDirs) {
    ...
    async listDirectory(dirPath, recursive = false, pattern) {
        try {
    ...
    async searchFiles(pattern, rootDir = '/', maxResults = 100) {
        try {
